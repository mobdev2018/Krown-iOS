...
@synthesize switchMen;
@synthesize switchWomen;
...

{
    ...
    slider = [[RangeSlider alloc] initWithFrame:CGRectMake(20,25,ScreenSize.width-40, 26)];
    
    // the slider enforces a height of 30, although I'm not sure that this is necessary
    
    slider.minimumRangeLength = 0.1;
    
    [slider setMinThumbImage:[UIImage imageNamed:@"Knob"]]; // the two thumb controls are given custom images
    
   
    [slider setMaxThumbImage:[UIImage imageNamed:@"Knob"]];
    
    [viewAge addSubview:slider];
...
{
    MenuViewController *setBackBool = [[MenuViewController alloc] init];
    setBackBool.BackBool=YES;
...
-(void)upDatePrefControls{
    ...
    if ([[UserSettings currentSetting].prSex intValue]==1) {
        ...
        switchMen.on = YES;
        switchMen.selected = YES;
        switchWomen.on = NO;
        switchWomen.selected = NO;
        ...
        switchMen.on = NO;
        switchMen.selected = NO;
        switchWomen.on = YES;
        switchWomen.selected = YES;
        prefGender = @"2";
    ...
    if (max>=58)
        ...
        lblAgeMin.text = [NSString stringWithFormat:@"Between %d to %d+",min,58];
    
    else
        
        lblAgeMin.text = [NSString stringWithFormat:@"Between %d to %d",min,max];
...
-(void)saveUpdatedValue
{
    ...
    if ([ud integerForKey:@"PrefMin"] ||[ud integerForKey:@"PrefMax"]) {
        ...
        lblAgeMin.text = [NSString stringWithFormat:@"Between %ld to %ld",(long)[ud integerForKey:@"PrefMin"],(long)[ud integerForKey:@"PrefMax"]];
        ...
        lblAgeMin.text = [NSString stringWithFormat:@"Between 18 to 58"];
...
- (void)report:(RangeSlider *)sender {
    ...
    if (max >= 58)
    {
        report = [NSString stringWithFormat:@"Between %d to 58+", min];
    }
    else
    {
        report = [NSString stringWithFormat:@"Between %d to %d", min, max];
    }
...
-(void)setSexs
{
    if (switchWomen.on && switchMen.on)
    {
        [UserSettings currentSetting].prSex = @"3";
    }
    else if (switchMen.on && switchWomen.on == NO)
    {
        [UserSettings currentSetting].prSex = @"1";
    }
    else if (switchWomen.on && switchMen.on == NO)
    {
