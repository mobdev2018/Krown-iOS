...
-(void)updatePresenceStatusOfFriend : (NSString *)friendName andStatus : (NSString *)currentStatus
{
    ...
    else
    {
        ...
        NSString *strDeviceID = [[NSUserDefaults standardUserDefaults] objectForKey:@"fbID"];
        NSString *myXMMPID =  [NSString stringWithFormat:@"%@%@",@"fb_", strDeviceID];
...
-(void)updatePendingMessageCounter : (NSString *)friendName isResetting : (BOOL)isResetCounter
{
    ...
    if([fetchedObjects count])
    {
        ...
        {
            NSInteger msgCount = [friendObject.messageCount integerValue];
            friendObject.messageCount = [NSNumber numberWithInteger:msgCount+1];
        }
