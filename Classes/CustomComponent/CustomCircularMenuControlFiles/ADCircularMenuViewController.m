...
-(void)setupUIOnView:(UIView *)view
{
...
- (void)setupButtons
{
    ...
    for (int i = 0; i < _iNumberOfButtons; i++)
    {
        ...
        UIImage *img = [UIImage imageNamed:[_arrButtonImageName objectAtIndex:i]];
        [button setImage:img forState:UIControlStateNormal];
        [button setBackgroundColor:[UIColor whiteColor]];
        [button.layer setCornerRadius:15];
        [button.layer setMasksToBounds:YES];
        if (!img) {
            [button setTitle:[_arrButtonImageName objectAtIndex:i] forState:UIControlStateNormal];
        }
