...
-(void)setData:(int)index dictForChat:(XmppFriend *)xmppFriend
{
   ...
   // self.imgOnLineOffLine.hidden = NO;
    ...
    UIFont *font = [UIFont systemFontOfSize:[[self.lblLastMsg font] pointSize]];
    NSMutableParagraphStyle *style = [[NSParagraphStyle defaultParagraphStyle] mutableCopy];
    [style setLineBreakMode:NSLineBreakByTruncatingTail];
    NSDictionary *sizeAttributes = @{NSFontAttributeName:font, NSParagraphStyleAttributeName: style};
    
    NSString *strMessage = xmppFriend.lastMessage;
    if (strMessage == nil) {
        strMessage = @"";
    }
    NSAttributedString *attributeText = [[NSAttributedString alloc] initWithString:strMessage attributes:sizeAttributes];
    CGRect rect = [attributeText boundingRectWithSize:(CGSize){maximumLabelSize.width, maximumLabelSize.height} options:NSStringDrawingUsesLineFragmentOrigin context:nil];
    CGSize expectedMsgSize = rect.size;
    
    //CGSize expectedMsgSize = [xmppFriend.lastMessage sizeWithFont:[UIFont systemFontOfSize:[[self.lblLastMsg font] pointSize]] constrainedToSize:maximumLabelSize lineBreakMode:NSLineBreakByTruncatingTail];
