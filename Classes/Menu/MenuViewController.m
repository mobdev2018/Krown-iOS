...
- (void)viewDidLoad
{
    ...
    //[Helper setToLabel:lblUserName Text:[[User currentUser]first_name] WithFont:HELVETICALTSTD_ROMAN FSize:18 Color:WHITE_COLOR];
    [Helper setToLabel:lblUserName Text:[[User currentUser]first_name] WithFont:HELVETICALTSTD_ROMAN FSize:17 Color:[UIColor colorWithRed:166.0/255.0 green:112.0/255.0  blue:98.0/255.0  alpha:1.0]];
    
    [vwContainer setFrame:CGRectMake(ScreenSize.width * 0.2 , vwContainer.frame.origin.y , vwContainer.frame.size.width - ScreenSize.width * 0.2 , vwContainer.frame.size.height)];
    
    [vwProfile setFrame:CGRectMake(0 , vwProfile.frame.origin.y , vwProfile.frame.size.width - ScreenSize.width * 0.2, vwProfile.frame.size.height)];
    
    NSLog(@"Values are %f", CGRectGetMidX(vwContainer.frame));
    
    CGRect frame_Profile_pic = imgProfilePic.frame;
    frame_Profile_pic.origin.x = (ScreenSize.width * 0.4) - (imgProfilePic.frame.size.width * 0.5);
    imgProfilePic.frame = frame_Profile_pic;
    
    
    CGRect frame_username = lblUserName.frame;
    frame_username.origin.x = (ScreenSize.width * 0.4) - (lblUserName.frame.size.width * 0.5);
    lblUserName.frame = frame_username;
    
    
    //[imgProfilePic setFrame:CGRectGetMidX(vwContainer.frame), imgProfilePic.frame.origin.y , vwProfile.frame.size.width - ScreenSize.width * 0.2, vwProfile.frame.size.height];
    [vwProfile layoutSubviews];
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    if (self.BackBool) {
        CGRect frame = vwContainer.frame;
...
{
    NSString *imageURL   =   [[NSString alloc] initWithFormat:@"http://graph.facebook.com/%@?fields=cover&access_token=%@",FB_Id,[FBSDKAccessToken currentAccessToken]];
...
- (CGFloat) getWidthOfString: (NSString *) string forSize:(CGSize) size
{
    ...
    NSAttributedString *attributeText = [[NSAttributedString alloc] initWithString:string attributes:sizeAttributes];
    CGRect rect = [attributeText boundingRectWithSize:size options:NSStringDrawingUsesLineFragmentOrigin context:nil];
...
- (void) scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (yPos > 0)
    {
        ...
        NSLog(@"Index %d yPos %f", (int)index, yPos);
