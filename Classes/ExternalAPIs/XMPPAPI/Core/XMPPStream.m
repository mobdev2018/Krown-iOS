...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag
{
	...
	XMPPLogRecvPre(@"RECIVEsocket: %@", [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]);
...
- (void)xmppParser:(XMPPParser *)sender didReadRoot:(NSXMLElement *)root
{
	...
	XMPPLogTrace();
	XMPPLogRecvPost(@"RECVeddidReadRoot: %@", [root compactXMLString]);
...
- (void)xmppParser:(XMPPParser *)sender didReadElement:(NSXMLElement *)element
{
 ...
 /* //sanket nagar
  ...
  //Update for error checking
    ...
    if (element) {
        NSArray *itemElements = [element elementsForName: @"error"];
        NSLog(@"erro %@",[itemElements description]);
        
        if([itemElements count] )
        {
            NSString *errorCode=[[[itemElements objectAtIndex:0] attributeForName:@"code"] stringValue];
            
            if([errorCode isEqualToString:@"409"] )
            {
                
                _errorCode = @"409";
               // Show_AlertView(@"Sign up", @"Username is already exist,");
                [[XmppCommunicationHandler sharedInstance] disconnect];
                return;
            }
        }
        
        //<iq xmlns="jabber:client" type="result" to="198.241.44.61/4a6b4381"/>
       // NSArray *itemElements1 = [element elementsForName: @"iq"];
       // NSLog(@"erro %@",[itemElements1 description]);
        
       // if([itemElements1 count])
        {
            NSString *type=[[element  attributeForName:@"type"] stringValue];
            NSString *to=[[element  attributeForName:@"to"] stringValue];

            
            if([type isEqualToString:@"result"] && to != nil)
            {
                
                _isRegisteredSuccessfully = YES;
...
//            
//            
//            NSDictionary *dictObj = [[NSDictionary alloc]initWithObjects:[NSArray arrayWithObjects:name,jid , nil] forKeys:[NSArray arrayWithObjects:@"friendName",@"friendJid", nil] ];
