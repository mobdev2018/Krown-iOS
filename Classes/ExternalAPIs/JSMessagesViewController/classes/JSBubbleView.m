...
-(NSString*) cacheDirectoryName {
    ...
    NSString *cacheDirectoryName = [documentsDirectory stringByAppendingPathComponent:@"FlamerProImages"];
...
- (CGSize)textSizeForText:(NSString *)txt
{
    if (![self isShowingForImage])    //Previous [self isText:txt]
    {
        ...
        NSDictionary *sizeAttributes = @{NSFontAttributeName:font};
        
        NSString *strText = self.textView.text;
        if (strText == nil) strText = @"";
        
        NSAttributedString *attributeText = [[NSAttributedString alloc] initWithString:strText attributes:sizeAttributes];
        CGRect rect = [attributeText boundingRectWithSize:(CGSize){maxWidth, maxHeight} options:NSStringDrawingUsesLineFragmentOrigin context:nil];
        return rect.size;
        
//        return [txt sizeWithFont:self.textView.font constrainedToSize:CGSizeMake(maxWidth, maxHeight)];
    }
    else{
...
- (CGSize)bubbleSizeForText:(NSString *)txt
{
	...
	return CGSizeMake(textSize.width + kBubblePaddingRight+50,
