...
- (void)configureAvatarImageView:(UIImageView *)imageView forMessageType:(JSBubbleMessageType)type
{
    ...
    if(type == JSBubbleMessageTypeOutgoing || type == JSBubbleMessageTypeOutgoingImage) {
    ...
    [imageView.layer setCornerRadius:kJSAvatarImageSize/2];
    
    
//    UIImageView  *AvatarBorder = [[UIImageView alloc] initWithFrame:CGRectMake(avatarX, avatarY, kJSAvatarImageSize, kJSAvatarImageSize)];
//    AvatarBorder.image =[UIImage imageNamed:@"ProfileMiniPicturePlaceholder"];
...
//    label.autoresizingMask = UIViewAutoresizingFlexibleWidth;
//    label.backgroundColor = [UIColor clearColor];
//    label.textAlignment = (type == JSBubbleMessageTypeOutgoing || type == JSBubbleMessageTypeOutgoingImage) ? NSTextAlignmentRight : NSTextAlignmentLeft;
//    label.textColor = [UIColor js_messagesTimestampColorClassic];
...
{
    self.bubbleView.font=[UIFont systemFontOfSize:13];
    self.bubbleView.text = msg;
}

- (void)setImageWithUrl:(NSString *)imgUrl
{
    [self.bubbleView setImageWithUrl:imgUrl];
}
...
{
   // [[UIPasteboard generalPasteboard] setString:[self.bubbleView text]];
    if ([self.delegate respondsToSelector:@selector(callForDeleteRowAtIndex:)]) {
        [self.delegate callForDeleteRowAtIndex:_indexOfRow];
    }
    [self resignFirstResponder];
}
