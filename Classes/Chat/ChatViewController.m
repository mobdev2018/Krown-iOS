...
@interface ChatViewController ()<MomentVCDelegate,CaptureMomentVCDelegate, UISearchDisplayDelegate, UISearchBarDelegate>
{
...
- (void)viewDidLoad
{
    ...
    self.searchBar.barTintColor = [UIColor colorWithRed:162.0/255 green:162.0/255 blue:162.0/255 alpha:1.0];
    ...
    [[UITextField appearanceWhenContainedIn:[UISearchBar class], nil] setBackgroundColor:[UIColor colorWithRed:213.0/255 green:214.0/255 blue:217.0/255 alpha:1.0]];
    ...
    [self.view setFrame:CGRectMake(0, 0 , ScreenSize.width - (ScreenSize.width * 0.2) , ScreenSize.height)];
    vwContainer.frame = CGRectMake(0, NavBarHeight, ScreenSize.width - (ScreenSize.width * 0.2) , ScreenSize.height-NavBarHeight);
    [vwTopbar setFrame:CGRectMake(0, 0 , ScreenSize.width - (ScreenSize.width * 0.2) , NavBarHeight)];
...
{
    self.MatchedCounterLbl.text = [NSString stringWithFormat:@"%d - Matches",(int)arrayXmppFriends.count];
    
    if (arrayMoments.count == 0)
    {
        vwMoments.hidden = YES;
        if (arrayXmppFriends.count == 0) {
            tblView.frame = CGRectMake(0,0, ScreenSize.width * 0.8f, ScreenSize.height-NavBarHeight);
    ...
    else
    {
        ...
        if (arrayXmppFriends.count == 0)
        {
           ...
           // _searchBar.hidden = YES;
            ...
            tblView.frame = CGRectMake(0, MomentViewHeight, ScreenSize.width * 0.8f, ScreenSize.height-frameMomentView.origin.y-MomentViewHeight);
         ...
         if (response)
         {
             ...
             if ([[response objectForKey:@"errFlag"] intValue]==0)
             {
                 ...
                 if (moments.count)
                 {
                     ...
                     lblMomentsCount.text = [NSString stringWithFormat:@"%lu moments",(unsigned long)moments.count];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell==nil)
    {
        cell=[[[NSBundle mainBundle]loadNibNamed:cellIdentiFier owner:tblView options:nil]lastObject];
        [cell.contentView setFrame:CGRectMake(0, 0, 100, 75)];
    }
    ...
    [cell setData:(int)indexPath.row dictForChat:xmppFriend];
    
    
    [cell.contentView setBackgroundColor:[UIColor colorWithRed:117.0/255.0 green:120.0/255.0 blue:123.0/255.0 alpha:1.0]];
...
-(void)hideMomentsDetailView:(NSMutableArray *)arrayMomentsUpdated
{
    ...
    if (arrayMoments.count)
    {
        ...
        lblMomentsCount.text = [NSString stringWithFormat:@"%d moments", (int)arrayMoments.count];
