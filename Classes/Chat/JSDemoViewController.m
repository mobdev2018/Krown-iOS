...
- (void)viewDidLoad
{
    ...
    [super viewDidLoad];
    UILabel *testingLabel = [[UILabel alloc] initWithFrame:CGRectMake(0,62,ScreenSize.width,1)];
    [testingLabel setBackgroundColor:[UIColor whiteColor]];
    ...
    UIImage *CamImage=[UIImage imageNamed:@"camera"];
    UIButton *CamBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [CamBtn setBackgroundImage:CamImage forState:UIControlStateNormal];
    CamBtn.frame = CGRectMake(14,11,28,22);
    [CamBtn addTarget:self action:@selector(takePhoto:) forControlEvents:UIControlEventTouchUpInside];
    
    self.messageInputView.textView.placeHolder = @"Message";
    [self.messageInputView addSubview:CamBtn];

    
  
//    UILabel *TimeAgo =[[UILabel alloc]initWithFrame:CGRectMake(100,40, 150, 30)];
//    [TimeAgo setTextAlignment:NSTextAlignmentCenter];
//    [TimeAgo setTextColor:[UIColor whiteColor]];
//    //TimeAgo.backgroundColor=[UIColor blackColor];
//    [TimeAgo setFont:[UIFont systemFontOfSize:11.0]];
//    TimeAgo.text=@"Active in 20 minutes";
...
-(void)setCustomNavBarTitleView
{
    ...
    titleView.textColor = [UIColor colorWithRed:166.0/255.0 green:112.0/255.0 blue:98.0/255.0 alpha:1.0];
    //titleView.shadowColor = [UIColor darkGrayColor];
    //titleView.shadowOffset = CGSizeMake(0, -1);
    titleView.text = currentChatObj.friend_DisplayName;
    ...
    if (currentChatObj.lastMessageTime.length)
            {
                ...
                subtitleView.text = difference;
        
            }
            else
            {
                subtitleView.text = @"Just Now";
            }

    //subtitleView.text = @"Active 20 minutes ago";
    subtitleView.adjustsFontSizeToFitWidth = YES;
    [_headerTitleSubtitleView addSubview:subtitleView];
    
    self.navigationItem.titleView = _headerTitleSubtitleView;
//    UIView *iv = [[UIView alloc] initWithFrame:CGRectMake(0,0,100,50)];
//    //iv.backgroundColor=[UIColor blackColor];
////    EGOImageView *imgUser =[[EGOImageView alloc]initWithFrame:CGctMake(0,5, 34, 34)];
//    UILabel *lblName =[[UILabel alloc]initWithFrame:CGRectMake(0, 0, 150, 30)];
//    //lblName.backgroundColor=[UIColor blackColor];
//    [lblName setTextAlignment:NSTextAlignmentCenter];
//    [lblName setTextColor:[UIColor colorWithRed:166.0/255.0 green:112.0/255.0 blue:98.0/255.0 alpha:1.0]];
...
//    }
    ...
    //TimeAgo.text=@"Active 20 minutes ago";
    
    //[iv addSubview:lblName];
    //[iv addSubview:TimeAgo];
...
-(void)addInitialPopupForNewMatched
{
    ...
    vwInitialPopupForNewMatched.backgroundColor=[UIColor clearColor];
    ...
    UIImageView *imgUserBorder =[[EGOImageView alloc]initWithFrame:CGRectMake(45, 80, 100, 100)];
    ...
    [vwInitialPopupForNewMatched addSubview:imgUserBorder];
...
//    }
    imgUser.image = [UIImage imageWithData:currentChatObj.profileImage];
    imgUserBorder.image = [UIImage imageNamed:@"ProfilePicturePlaceholder"];
...
{
    UIImage *imgButton = [UIImage imageNamed:@"flag"];
...
{
    UIImage *imgButton = [UIImage imageNamed:@"BackArrow.png"];
	UIButton *rightbarbutton = [UIButton buttonWithType:UIButtonTypeCustom];
	[rightbarbutton setFrame:CGRectMake(0,4,12,21)];
...
-(void)sendMesasage
{
        ...
        NSString *msgWithTime = [temp stringByAppendingString:[NSString stringWithFormat:@"%d",(int)timestampMin]];
                       ...
                       forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(![senderName isEqualToString:MY_USER_NAME])
    {
        return  [JSBubbleImageViewFactory bubbleImageViewForType:type color:[UIColor js_iOS7IncomingBubbleColor]];
    }
    else
    {
        return [JSBubbleImageViewFactory bubbleImageViewForType:type color:[UIColor js_iOS7OutgoingBubbleColor]];
    }
}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if(buttonIndex == 1)
    {
        if (alertView.tag == 100)
        {
            ...
            [dictParam setObject:[NSString stringWithFormat:@"%d", (int)EntFlagUnblock] forKey:PARAM_ENT_FLAG];
        ...
        {/*block user service call */
            ...
            [dictParam setObject:[NSString stringWithFormat:@"%d", (int)EntFlagBlock] forKey:PARAM_ENT_FLAG];
            [dictParam setObject:[User currentUser].fbid forKey:PARAM_ENT_USER_FBID];
            [dictParam setObject:fbId forKey:PARAM_ENT_USER_BLOCK_FBID];
...
//    NSString *msgWithTime = [temp stringByAppendingString:[NSString stringWithFormat:@"%d", (int)timestampMin]];
    ...
    NSString *msgToSave = [sendMsgStr stringByAppendingString:[NSString stringWithFormat:@"#$%d", (int)timestampMin]];
    
    
    //update for configure woth android
    
    NSString *msgWithImgUrlAndTime = [NSString stringWithFormat:@"TindercloneImage_%@#$%@",sendMsgStr, [NSString stringWithFormat:@"%d", (int)timestampMin]];
