...
-(void)setHelpScreens
{
    ...
    self.scrImages.contentSize = CGSizeMake(screenSize.width * arrImages.count, self.scrImages.frame.size.height);
...
-(void)setScrollViewForImage
{
    ...
    {
        CGRect frame;
        frame.origin.x = screenSize.width * i;
        frame.origin.y = 0;
        frame.size = CGSizeMake(screenSize.width, self.scrImages.frame.size.height);
...
-(IBAction)changePage:(id)sender
{
    ...
    frame.origin.y = 0;
    frame.size = CGSizeMake(screenSize.width, frame.size.height);
...
-(IBAction)onClickDone:(id)sender
{
    ...
    NSUInteger unitFlags = NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay; //|NSHourCalendarUnit|NSMinuteCalendarUnit|NSSecondCalendarUnit;
    ...
    NSLog(@"%d Years , %d Months , %d Days", (int)years, (int)months, (int)days);
    ...
    else
    {
        ...
        else
        {
            ...
            [login logInWithReadPermissions: @[@"user_birthday",@"user_events",@"email",@"public_profile",@"user_photos"]
...
-(void)getFacebookUserDetails
{
    ...
    if ([[FacebookUtility sharedObject] isLogin])
    {
        NSString *fields = @"id,birthday,gender,first_name,age_range,last_name,name,picture.type(large),cover";
        [[FacebookUtility sharedObject] fetchMeWithFields:fields FBCompletionBlock:^(id response, NSError *error)
         {
             if (!error)
             {
...
-(void)parseLogin :(NSDictionary*)FBUserDetailDict
{
    ...
    [dictParam setObject:[NSString stringWithFormat:@"%d",(int)sex] forKey:PARAM_ENT_SEX];
